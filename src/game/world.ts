...
export function triggerRandomWorldEvent(): WorldEvent | null {
  const db = getDb();
  const activeCount = (db.prepare('SELECT COUNT(*) as count FROM world_events WHERE active = 1').get() as { count: number }).count;
  ...
  `).get(result.lastInsertRowid) as WorldEvent;
}

export function getWorldEventMultipliers(
  playerRegion: string,
  ...
  for (const event of activeEvents) {
    if (event.region && event.region !== playerRegion) continue;
    try {
      const effect = JSON.parse(event.effect);
      if (effect.silverMultiplier) multipliers.silverMultiplier *= effect.silverMultiplier;
      if (effect.knowledgeMultiplier) multipliers.knowledgeMultiplier *= effect.knowledgeMultiplier;
      if (effect.xpMultiplier) multipliers.xpMultiplier *= effect.xpMultiplier;
      if (effect.threatMultiplier) multipliers.threatMultiplier *= effect.threatMultiplier;
      if (effect.resourceMultiplier) multipliers.resourceMultiplier *= effect.resourceMultiplier;
      if (effect.visibilityIncrease) multipliers.visibilityIncrease += effect.visibilityIncrease;
    } catch {
...
export function getWorldState() {
  ...
  const playersByRegion: Record<string, number> = {};
  for (const region of REGIONS) {
    const count = (db.prepare('SELECT COUNT(*) as count FROM players WHERE region = ? AND alive = 1').get(region.name) as { count: number }).count;
  ...
  const recentEvents = db.prepare(`
    ...
    LIMIT 20
  `).all(now - 300) as Array<{ type: string; message: string; createdAt: number }>;
