import { CharacterClass, CLASS_DEFINITIONS } from './character';
...
export interface EncounterResult {
  ...
  damageDealt?: number;
  damageTaken?: number;
  reward?: { silver: number; xp: number };
  message: string;
}
...
export function calculateThreatChance(
  ...
  const REGION_THREAT_MULTIPLIERS: Record<string, number> = {
...
export function selectThreat(visibility: number): Threat {
  const eligible = THREATS.filter((t) => t.visibilityRequired <= visibility);
  if (eligible.length === 0) return THREATS[0];
...
export function resolveEncounter(
  ...
  const playerDamageDealt = Math.max(
    1,
    playerAttack - Math.floor(threat.attack * 0.3) + Math.floor(Math.random() * 10)
  );
  const damageTaken = Math.max(
    1,
    Math.floor(
      (threat.damage - Math.floor(playerDefense * 0.4)) * damageReduction +
...
export function calculateAttack(
  ...
  defenderSilver: number
): AttackResult {
  const attackerDef = CLASS_DEFINITIONS[attackerClass];
  ...
  const damage = Math.max(
    5,
    attackerAttack * 1.5 - defenderDefense * 0.5 + Math.random() * 20
  );
...
export function canDetect(
  detectorWisdom: number,
  detectorClass: CharacterClass,
  targetVisibility: number,
  targetStealth: number
): boolean {
  const classDef = CLASS_DEFINITIONS[detectorClass];
