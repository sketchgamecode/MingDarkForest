import { CharacterClass, CLASS_DEFINITIONS, getVisibilityTier } from './character';
...
export const REGION_BONUSES: Record<string, Partial<Resources> & { threatMultiplier: number }> = {
...
export function calculateResourceTick(
  ...
  const regionBonus = REGION_BONUSES[region] || REGION_BONUSES['Imperial Capital'];

  const stanceMultiplier = stance === 'hide' ? 0.5 : stance === 'expand' ? 1.8 : 1.0;
  const levelMultiplier = 1 + (level - 1) * 0.1;

  const visibilityTier = getVisibilityTier(visibility);
  ...
  return {
    silver:
      ...
      ticksPerSecond,
    knowledge:
      classDef.resourceRates.knowledge *
      ...
      ticksPerSecond,
    supplies:
      classDef.resourceRates.supplies *
    ...
    qi:
      ...
      ticksPerSecond,
    xp: Math.floor(
      (level * 2 + visibilityTier * 3) * stanceMultiplier * ticksPerSecond
    ),
  };
}
...
export function calculateOfflineProgress(
  ...
  if (elapsed <= 0) return { silver: 0, knowledge: 0, supplies: 0, qi: 0, xp: 0 };

  const tick = calculateResourceTick(
    playerClass,
    level,
    stance,
    region,
...
export function canAffordUpgrade(
  ...
  return (
    resources.silver >= upgrade.cost.silver * costMultiplier &&
