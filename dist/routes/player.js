...
const idle_1 = require("../game/idle");
const darkForest_1 = require("../game/darkForest");
...
router.post('/', (req, res) => {
    ...
    if (!character_1.CLASS_DEFINITIONS[characterClass]) {
    ...
    const classDef = character_1.CLASS_DEFINITIONS[characterClass];
    const stats = (0, character_1.applyLevelBonuses)(classDef.baseStats, 1);
    ...
    `).run(id, name, characterClass, stats.maxHealth, stats.maxHealth, stats.attack, stats.defense, stats.wisdom, stats.stealth, classDef.startingVisibility, now, now, now);
        db.prepare(`
...
router.put('/:id/action', (req, res) => {
    ...
    if (!player)
        return res.status(404).json({ error: 'Player not found' });
    if (!player.alive)
        return res.status(400).json({ error: 'Your character has died. Create a new one.' });
    const now = Math.floor(Date.now() / 1000);
    ...
    switch (action) {
        case 'hide': {
            ...
            db.prepare("INSERT INTO events (type, message, player_id) VALUES ('action', ?, ?)").run(`${player.name} retreated into the shadows`, player.id);
        ...
        case 'expand': {
            ...
            db.prepare("INSERT INTO events (type, message, player_id) VALUES ('action', ?, ?)").run(`${player.name} expanded their presence boldly`, player.id);
        ...
        case 'move': {
            if (!regionName || !world_1.REGIONS.find(r => r.name === regionName)) {
            ...
            db.prepare("INSERT INTO events (type, message, player_id) VALUES ('action', ?, ?)").run(`${player.name} traveled to ${regionName}`, player.id);
        ...
        case 'attack': {
            ...
            const targetPlayer = db.prepare('SELECT * FROM players WHERE id = ? AND alive = 1').get(target);
            if (!targetPlayer)
            ...
            const canDetectTarget = (0, darkForest_1.canDetect)(player.wisdom, player.class, targetPlayer.visibility, targetPlayer.stealth);
            ...
            const result = (0, darkForest_1.calculateAttack)(player.class, player.attack, player.level, targetPlayer.class, targetPlayer.defense, targetPlayer.health, targetPlayer.stealth, targetPlayer.silver);
            if (result.success) {
                const targetDead = result.damageDealt >= targetPlayer.health;
                db.prepare('UPDATE players SET health = MAX(0, health - ?), silver = MAX(0, silver - ?), alive = ? WHERE id = ?').run(result.damageDealt, result.silverStolen, targetDead ? 0 : 1, targetPlayer.id);
                db.prepare('UPDATE players SET silver = silver + ?, xp = xp + ?, kills = kills + ? WHERE id = ?').run(result.silverStolen, result.xpGained, targetDead ? 1 : 0, player.id);
                if (targetDead) {
                    db.prepare('UPDATE players SET deaths = deaths + 1 WHERE id = ?').run(targetPlayer.id);
                    db.prepare("INSERT INTO events (type, message, player_id) VALUES ('pvp_kill', ?, ?)").run(`${player.name} slew ${targetPlayer.name} in combat!`, player.id);
                }
            }
            return res.json({ success: result.success, message: result.message });
        }
        case 'scout': {
            const classDef = character_1.CLASS_DEFINITIONS[player.class];
      ...
      `).all(player.region, player.id);
            const detected = nearbyPlayers.filter(p => (0, darkForest_1.canDetect)(player.wisdom, player.class, p.visibility, 0));
            return res.json({
                success: true,
        ...
        case 'upgrade': {
            if (!upgradeId || !idle_1.UPGRADES[upgradeId]) {
            ...
            if (!(0, idle_1.canAffordUpgrade)(resources, upgradeId, currentLevel)) {
                return res.status(400).json({ error: 'Cannot afford this upgrade or already at max level' });
            }
            const newResources = (0, idle_1.applyUpgradeCost)(resources, upgradeId, currentLevel);
      ...
      `).run(newResources.silver, newResources.knowledge, newResources.supplies, newResources.qi, JSON.stringify(upgrades), player.id);
            return res.json({
                success: true,
                message: `Upgraded ${idle_1.UPGRADES[upgradeId].name} to level ${currentLevel + 1}!`,
                newLevel: currentLevel + 1,
            });
        }
        case 'revive': {
            if (player.alive)
                return res.status(400).json({ error: 'You are still alive' });
...
router.get('/:id/collect', (req, res) => {
    ...
    const upgrades = JSON.parse(player.upgrades || '{}');
    const offlineGain = (0, idle_1.calculateOfflineProgress)(player.class, player.level, player.stance, player.region, player.visibility, upgrades, player.last_tick);
    const now = Math.floor(Date.now() / 1000);
    ...
    UPDATE players SET
      ...
      last_tick = ?
    WHERE id = ?
  `).run(offlineGain.silver, offlineGain.knowledge, offlineGain.supplies, offlineGain.qi, newXp, newLevel, now, player.id);
...
function formatPlayer(player) {
    ...
    const xpForNext = (0, character_1.getXpRequired)(level);
    return {
        id: player.id,
        name: player.name,
        class: player.class,
        ...
        visibilityDescription: (0, character_1.getVisibilityDescription)(player.visibility),
